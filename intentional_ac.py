...
class IntentionalAC(nn.Module):
    def __init__(self, n_obs=11, n_actions=3, hidden_size=128, gamma=0.99, lamda=0.8, eta_policy=0.05, eta_value=0.5):
    ...
    def update_params(self, s, a, r, s_prime, done, terminated, entropy_coeff):
        ...
        s, a, r, s_prime, termination_mask = torch.tensor(np.array(s), dtype=torch.float), torch.tensor(np.array(a)), \
                                         torch.tensor(np.array(r)), torch.tensor(np.array(s_prime), dtype=torch.float), \
                                         torch.tensor(np.array(termination_mask), dtype=torch.float)
...
def main(env_name, seed, gamma, lamda, total_steps, entropy_coeff, eta_policy, eta_value, debug, render=False, track=False, wandb_project="intentional-updates", log_metrics=False, log_interval=1000, log_dir="logs", dtype_tag="fp32", stop_on_nonfinite=False):
    ...
    agent = IntentionalAC(n_obs=env.observation_space.shape[0], n_actions=env.action_space.shape[0], gamma=gamma, lamda=lamda, eta_policy=eta_policy, eta_value=eta_value)
    if track:
        import wandb
        wandb.init(
    ...
    logger = None
    if log_metrics:
        run_name = f"intentional_ac_{env.spec.id}_seed{seed}_{dtype_tag}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dtype_tag", type=str, default="fp32")
    parser.add_argument("--stop_on_nonfinite", action="store_true")
    args = parser.parse_args()
    main(args.env_name, args.seed, args.gamma, args.lamda, args.total_steps, args.entropy_coeff, args.eta_policy, args.eta_value, args.debug, args.render, args.track, args.wandb_project, args.log_metrics, args.log_interval, args.log_dir, args.dtype_tag, args.stop_on_nonfinite)
