...
def main(env_name, seed, gamma, lamda, total_steps, epsilon_target, epsilon_start, exploration_fraction, eta_value, debug, render=False, track=False, wandb_project="intentional-updates", log_metrics=False, log_interval=1000, log_dir="logs", dtype_tag="fp32", stop_on_nonfinite=False):
    ...
    logger = None
    if log_metrics:
        run_name = f"intentional_q_minatar_{env.spec.id}_seed{seed}_{dtype_tag}"
